...
@Slf4j
public class ReplyResctController {
	
	private final ReplyMapper replyMapper;
	
	//리플 등록
	@PostMapping("{board_id}")  
	public ResponseEntity<String> writeReploy(@ModelAttribute Reply reply,
	...
	public ResponseEntity<List<Reply>> findReplies(@PathVariable Long board_id){
		...
		return ResponseEntiry.ok(replies);
		
	}
	
	//리플수정
	@PutMapping("{board_id}/{reply_id}")
	public ResponseEntity<Reply> updateReply(@SessionAttribute("loginMebmer")Member loginMember,
											 @PathVariable Long board_id,
											 @PathVariable Long reply_id,
	...
	public ResponseEntity<> removeReply(@SessionAttribute("loginMember") Member loginMember,
							  		    @PathVariable Long board_id,
							  		    @PathVariable Long reply_id){
		//삭제권한이 있는지 체크 
